...
<template>
  <div class="gis-root">
    ...
    <!-- ── TOP TOOLBAR ── -->
    <div class="toolbar">
      <span class="brand">▣ MINI GIS</span>
...
async function handleImport(e) {
  ...
  const groups = {}
  files.forEach(f => {
    const base = f.name.replace(/\.(shp|dbf|shx|prj)$/i, '')
    if (!groups[base]) groups[base] = {}
    groups[base][f.name.split('.').pop().toLowerCase()] = f
  })

  for (const [name, group] of Object.entries(groups)) {
    if (!group.shp) { showStatus(`No .shp for "${name}"`); continue }
    try {
      const shpBuf = await readFileBuffer(group.shp)
      const dbfBuf = group.dbf ? await readFileBuffer(group.dbf) : null
      let geojson
      if (dbfBuf) {
        geojson = await shp.combine([shp.parseShp(shpBuf), shp.parseDbf(dbfBuf)])
      } else {
        const parsed = shp.parseShp(shpBuf)
        geojson = { type: 'FeatureCollection', features: parsed.map((g, i) => ({ type: 'Feature', geometry: g, properties: { id: i } })) }
      }
      const id = newLayerId()
      const color = LAYER_COLORS[layers.value.length % LAYER_COLORS.length]
      layers.value.push({ id, name: name.split('/').pop(), geojson, visible: true, color })
      activeLayerId.value = id
      showStatus(`Imported "${name}" — ${geojson.features?.length || 0} features`)
    } catch (err) {
      showStatus(`Error importing "${name}": ${err.message}`)
    }
  }
}
